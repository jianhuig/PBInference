# The purpose of this is to rewrite Chen Chen so that it is more general

chen_chen <- function(Y, Xlab, flab, fall, Xall) {
  # Sample sizes
  n_labeled <- length(Y) # Number of labeled samples
  n_total <- n_labeled + length(fall) # Total number of samples (labeled + unlabeled)
  ...
  D1 <- -crossprod(Xlab_int, Xlab_int) / n_labeled
  ...
  Omega <- (D1_inv %*% C11 %*% D1_inv - (1 - n_labeled / n_total) * D1_C12_C22 %*% t(D1_C12)) / n_labeled
